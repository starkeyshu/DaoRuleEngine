...
public class StatefulRuleSessionImpl implements StatefulRuleSession {
  ...
  public StatefulRuleSessionImpl(RuleExecutionSetImpl ruleset, Map properties) {
    ...
    this.ruleAdmin=new RuleAdmin(ruleSet.getRules());//Assert rules not empty
  }
  ...
  public final Handle addObject(Object object) throws InvalidRuleSessionException {
    try {
      ...
      if (object instanceof Term)
        workingMemory.put(((Term)object).getName(),((Term)object).getValue());
        ...
        //FIXME add an entry for each super-class...
        /*
  ...
  public final void executeRules() throws InvalidRuleSessionException {
    validateRuleSession();
    try {
      forwardChaining(new HashSet(),ruleAdmin.getRulesByDepends(null));
    }
    catch(Exception ex) {
      throw new InvalidRuleSessionException("Internal error", ex);
    }
  ...
  /**
   * Verify that there exits a rule execution set.
   */
        ...
        if (methods[i].getName().equals(method)) {
          if (action.getValues().size()==methods[i].getParameterTypes().length) {
            ...
            if (obj instanceof Term)
              workingMemory.put(((Term)obj).getName(),((Term)obj).getValue());
   ...
   */
  private void forwardChaining(HashSet rulesAlreadyFired, List toFireRules) throws InvalidRuleSessionException {
    RuleImpl rule = null;
    Assumption ass = null;
    Action action = null;
    String t1, t2;
    ...
    for(int i=0;i<toFireRules.size();i++) {
      rule = (RuleImpl)toFireRules.get(i);
      //System.out.println(i+":"+rule+":"+rulesAlreadyFired);
      if (rulesAlreadyFired.contains(rule))
        continue;
      variables.clear();
      ok = true;
...
//          }
      ...
      List subRules=ruleAdmin.getRulesByDepends(rule.getCode());
      if(subRules!=null&&subRules.size()>0){
    	  forwardChaining(rulesAlreadyFired,subRules);
      }
      //FIXME After rule firing all rules are re-analyzed.
